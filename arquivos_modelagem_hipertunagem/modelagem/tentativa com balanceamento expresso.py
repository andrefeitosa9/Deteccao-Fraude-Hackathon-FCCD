...
def validacao_cruzada(folds, threshold, params, df_teste):
    ...
    for fold_idx, (X_treino, y_treino, X_teste, y_teste) in enumerate(folds):
        ...
        # Ajuste manual dos pesos das classes
        class_weights = np.where(y_treino == 1, len(y_treino) / np.sum(y_treino == 1), len(y_treino) / np.sum(y_treino == 0))
        modelo.fit(X_treino, y_treino, sample_weight=class_weights)
